...
| RF-002 | Đã xử lý: `GET /api/internal/runtime/product-repository` yêu cầu session `ADMIN`/`SUPER_ADMIN` | SEC-01, AR-07 | 1 | S |
...
| RF-005 | Đã xử lý: `loginWithCredentials` dùng `LoginSchema.safeParse` và shared credentials verification | SEC-06 | 1 | S |
...
| RF-105 | Đã xử lý: auth rate limit login/register/resend | Chống brute force credentials, registration flooding và resend abuse | `auth/actions/*`, `auth/lib/rate-limit/*` | S | Done |
...
| RF-202 | Prisma list select gọn | Soft-delete filtering đã xử lý; còn giảm over-fetch | P2 |
...
| RF-205 | Đã xử lý: cart merge không nhận/tin `userId` client | `mergeCart({ guestCart })` lấy ownership từ session | Done |
...
| RF-220 | Error page | `/forbidden` đã có; `/error` page vẫn chưa có | P3 |
...
- RF-102
- RF-103
- RF-105 đã xử lý cho auth; cart rate limit vẫn thuộc RF-204
- RF-201
- RF-205
...
- RF-106
- RF-107
- RF-202 phần soft-delete đã xử lý; over-fetch còn lại
- RF-203
- RF-206
- RF-207
- RF-212
